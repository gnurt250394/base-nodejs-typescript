...
export abstract class BaseRepository<EntityType> implements IBaseRepository<EntityType> {
  ...
  public async exists(query: any): Promise<boolean> {
    const document = await this.documentModel.exists(query);
    return document;
  }
  ...
  public async delete(_id: string): Promise<{ ok?: number; n?: number } & { deletedCount?: number }> {
    return this.documentModel.deleteOne({ _id });
  }

  public async find(skip: number = 0, limit: number = 250, sort: string, query: any): Promise<EntityType[]> {
  ...
  public async findOneAndUpdate<T>(query: any, update: any, options: any): Promise<EntityType> {
    const document = await this.documentModel.findOneAndUpdate(query, update, options);
    if (!document) throw new ApiError(constants.errorTypes.validation);
    return new this.formatter(document);
  }
