import { Sequelize, IncludeOptions, FindOptions, Order, Op } from 'sequelize';
...
export abstract class BaseRepository<EntityType> implements IBaseRepository<EntityType> {
  ...
  protected getInclude: IncludeOptions[] = [];
  protected saveInclude: IncludeOptions[] = [];
  ...
  public async find(offset: number = 0, limit: number = 250, sort: string, query: any, include = this.getInclude): Promise<EntityType[]> {
    ...
    if (order) options.order = order as Order;
    return (await this.entityModel.model.findAll(options)).map(item => new this.formatter(item));
  }
  ...
  protected whereQueryFormatter = (key: string, value: any): any => {
    ...
    if (value instanceof Array)
      return {
        [Op.or]: value.map(v => this.whereQueryFormatter(key, v)),
      };
    else if (isId(key)) return { [Op.eq]: value };
    else if (typeof value === 'string') return { [Op.like]: `%${value}%` };
    else return { [Op.eq]: value };
  };
